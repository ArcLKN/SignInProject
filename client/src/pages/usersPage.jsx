...
import {
    Box, Center, HStack, Text, Flex, IconButton
...
export default function Users() {
    ...
    useEffect(() => {
        const initData = async () => {
			try {
				const response = await window.fetch('http://localhost:3001/api/get-users');
                ...
                setUsers(allUsers);
                setSortedUsers(Object.fromEntries(Object.entries(allUsers).slice(0, 14)))
    ...
    const [maxPages, setMaxPages] = useState(Math.ceil(Object.entries(mockupUsers).length/userPerPage))
    ...
    function getSortedList({
        ...
        newSortedUsers = Object.fromEntries(
            Object.entries(newSortedUsers).filter(
                ([key, value]) => value.email.toLowerCase().includes(updatedSearchFilter) ||
        ...
        if (updatedUsersPerPage == Infinity) {
            ...
            setMaxPages(Math.ceil(Object.entries(newSortedUsers).length/updatedUsersPerPage))
        }
        newSortedUsers = Object.fromEntries(Object.entries(newSortedUsers).slice(
    ...
    const getApiAddUser = async (event) => {
        try {
            const response = await window.fetch('http://localhost:3001/api/add-user', {
                method: 'POST',
                headers: {
    ...
    function changeUsersPerPage(event) {
        ...
        getSortedList({updatedUsersPerPage: Number(event.target.value), updatedActualPage: 1})
    }
    ...
    return (
        <Box p="7" maxW="100vw" height="100vh">
            <AddUserModal isOpen={doShowAddUserModal} doOpen={setDoShowAddUserModal} createNewUser={createNewUser}/>
            <Flex direction={"column"} h="100%">
                <Box mb="5">
                    <Flex align="center">  
                        <HStack spacing="24px">
                            <Text fontSize="4xl" fontWeight="bold">Users</Text>
                ...
                <Center maxHeight="90%">
                    <Box rounded="20px" borderWidth="1px" w="100%" h="100%" color={styleVariables.colors.ligrey}>
                        <Flex direction={"column"} h="100%">
                            <NavUsersTable
