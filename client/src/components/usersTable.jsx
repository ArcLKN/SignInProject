  ...
  const listTableUsersValues = Object.entries(sortedUsers).map(
    ([userId, value]) => (
      <Tr
        key={userId}
        w="100%"
        bgColor={selectedRows[userId] ? "gray.200" : "white"}
      >
        <Th
          key={"checkbox" + userId}
          border={"1px solid"}
        ...
        {Object.entries(
          Object.fromEntries(
            Object.keys(value)
              .filter((key) => Object.keys(mockupUsersKeys).includes(key))
        ...
        >
          <Menu>
            <MenuButton rounded={"0px"} w="100%" as={Button} bgColor={"white"}>
              <Icon>
                <HamburgerIcon />
              </Icon>
            </MenuButton>
            ...
            >
              <MenuGroup title="Manage user">
                <MenuItem
                  p="2"
                  bgColor={"red.200"}
                  onClick={() => deleteUser(userId, value._id)}
                >
                  Delete
                </MenuItem>
              </MenuGroup>
